...
Analyzes images/videos using LLMProvider API to automatically categorize, tag, and extract
semantic information. All processing is CPU-only via remote LLM APIs.

Features:
  - Image analysis using LLMProvider 3.5 Sonnet with vision capabilities
...
# ============================================================================
# LLMProvider API Client
# ============================================================================

class LLMProviderVisionAnalyzer:
    """
    LLMProvider API client for image analysis using vision capabilities.

    Uses LLMProvider 3.5 Sonnet with vision for comprehensive image understanding.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, max_retries: int = 3):
        ...
        Args:
            api_key: LLMVendor API key (defaults to LLM_VENDOR_API_KEY env var)
            max_retries: Maximum retry attempts for failed requests
        """
        self.api_key = api_key or os.environ.get('LLM_VENDOR_API_KEY')
        if not self.api_key:
            raise ValueError(
                "LLMVendor API key required. Set LLM_VENDOR_API_KEY environment variable "
        ...
        # Rate limiting (50 requests per minute for LLMVendor)
        ...
        try:
            ...
            raise ImportError(
                "llm_vendor package not installed. Install with: pip install llm_vendor"
            )
    ...
    def _encode_image(self, image_path: Path) -> str:
        """
        Encode image to base64 for LLMProvider API.
    ...
    def analyze_image(
        ...
        """
        Analyze image using LLMProvider vision API.
...
Respond with ONLY the JSON object, no additional text."""
    ...
    def _parse_response(self, response_text: str) -> Dict[str, Any]:
        """
        Parse LLMProvider API response into structured format.

        Args:
            response_text: Raw response text from LLMProvider
        ...
        try:
            ...
            # LLMProvider might wrap JSON in markdown code blocks
...
def categorize_images(
    ...
    # Initialize analyzer
    analyzer = LLMProviderVisionAnalyzer(api_key=api_key)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Auto-categorize images using LLMProvider API with vision'
    )
    ...
    parser.add_argument('--api-key', type=str,
                       help='LLMVendor API key (defaults to LLM_VENDOR_API_KEY env var)')
